package pe.crediya.autenticacion.api.exception;
...
import pe.crediya.autenticacion.api.dto.ErrorResponseDto;
...
import pe.crediya.autenticacion.model.usuario.exception.UsuarioYaExisteException;
...
public class GlobalExceptionHandler implements WebExceptionHandler {
    ...
    private void logError(Throwable ex, String method, String path) {
        if (ex instanceof DomainValidationException ) {
            log.error("Error de dominio en {} {}: {}", method, path, ex.getMessage());
        }
        else if (ex instanceof ServerWebInputException || ex instanceof IllegalArgumentException) {
            log.error("Error de validación en {} {}: {}", method, path, ex.getMessage());
        } else {
    ...
    private HttpStatus determineHttpStatus(String codigo) {
        return switch (codigo) {
            case "DOMAIN_VALIDATION_ERROR" -> HttpStatus.BAD_REQUEST;
            case "USER_NOT_FOUND" -> HttpStatus.NOT_FOUND;
            case "OPERATION_NOT_ALLOWED" , "CREATE_USER_ERROR"-> HttpStatus.CONFLICT;
    ...
    private ErrorResponseDto mapToError(Throwable ex, String path) {
        ...
        if(ex instanceof UsuarioYaExisteException e) {
            return ErrorResponseDto.builder()
                    .codigo("CREATE_USER_ERROR")
                    .mensaje("Error de validación de dominio")
                    .detalles(e.getMessage())
                    .timestamp(LocalDateTime.now())
                    .path(path).build();
        } else if (ex instanceof DomainValidationException e) {
            return ErrorResponseDto.builder()
                    ...
                    .timestamp(LocalDateTime.now())
                    .path(path).build();
        } else if (ex instanceof TokenInvalidoException e) {
            return ErrorResponseDto.builder()
